...
import { runDingTalkLeaveCheck } from '../api'
import {
...
type DingTalkPatrolBlockProps = {
  form: UseFormReturn<OAuthFormValues>
}

export function DingTalkPatrolBlock(props: DingTalkPatrolBlockProps) {
  const { t } = useTranslation()
  const patrolEnabled = props.form.watch('dingtalk.patrol_enabled')
  const patrolMode = props.form.watch('dingtalk.patrol_mode')

  const runCheck = useMutation({
    mutationFn: runDingTalkLeaveCheck,
  })
        ...
        render={({ field }) => (
          <SettingsSwitchItem>
            <SettingsSwitchContent>
              ...
              <FormDescription>
                {t(
                  'Periodically check DingTalk-bound users and automatically disable the account and API tokens of employees who left the organization'
                )}
              </FormDescription>
            </SettingsSwitchContent>
            <FormControl>
      ...
      {patrolEnabled ? (
        <>
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>
                  {t(
                    'Departed employees are only disabled when DingTalk explicitly confirms they left; unknown states are skipped'
                  )}
                </FormDescription>
                <FormMessage />
              </FormItem>
          ...
          {patrolMode === 'daily' ? (
            ...
            <FormField
              control={props.form.control}
              name='dingtalk.patrol_interval_hours'
              render={({ field }) => (
                <FormItem>
